...
void Shader::Compile() {
    ...
    try {
        ...
        LW_LOG("Shader: Compiled '", m_Path, "' new version=", m_Version, " gpuID=", m_GpuID);

        ExtractSamplers();
    }
    catch (const std::exception& e) {
        LW_ERROR("Shader: Error while compiling '", m_Path, "':\n", e.what());
...
void Shader::SetUniformBlock(const std::string& blockName, const int bindingPoint) {
	...
	if (const uint32_t blockIndex = glGetUniformBlockIndex(m_GpuID, blockName.c_str());
		...
		LW_WARN("Shader: Block '", blockName, "' not found in shader '", m_Path, "'");
	}
}

int Shader::GetUniformLocation(const std::string_view name) const {
	LW_ASSERT(m_GpuID != 0, "Shader: Cannot get a uniform location, GPU ID is 0.");
