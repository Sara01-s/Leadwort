...
namespace Leadwort::Rendering::Bindables {
	...
	Mesh::Mesh(const MeshData& meshData, AssetManagement::AssetKey<Mesh>&&) noexcept
		: m_MeshData(meshData)
	{
		LW_ASSERT(m_MeshData.material != nullptr, "Mesh constructor: Material is NULL after initialization!");
	...
	Mesh::~Mesh() {
		LW_LOG("Destroying Mesh with GPU ID: ", m_GpuID);
	...
	void Mesh::Render() const noexcept {
		if (m_IndexCount <= 0) {
			LW_WARN("Trying to render a mesh with IndexCount = 0!");
			return;
		}
