...
#define CORE_ENABLE_ASSERTS_IN_RELEASE 1
	...
	#define LW_ASSERT(statement, message)                                                                            \
		if (!(static_cast<bool>(statement))) {                                                                         \
			Leadwort::Utils::Log::Error(__FILE__, ":", __LINE__, ":\nAssertion failed: ", #statement, ": ", message);    \
			assert(0);                                                                                                 \
		}

	#define LW_LOG(...)   Leadwort::Utils::Log::Info(__VA_ARGS__)
	#define LW_WARN(...)  Leadwort::Utils::Log::Warn(__VA_ARGS__)
	#define LW_ERROR(...) Leadwort::Utils::Log::Error(__VA_ARGS__)
#else
