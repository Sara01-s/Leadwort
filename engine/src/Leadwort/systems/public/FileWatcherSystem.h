...
public:
    ...
    int Watch(const fs::path& directory, FileChangeCallback callback) {
        ...
        if (!fs::exists(normalized)) {
            LW_ERROR("FileWatcherSystem: directory does not exist: ", normalized.string());
            return -1;
        }
        ...
        if (dirIt != m_DirToWatchId.end()) {
            ...
            LW_LOG("FileWatcherSystem [Watch]: reusing existing watchId=", watchId,
                     " for dir='", normalized.string(), "'");
        }
        else {
            watchId = m_FileWatcher.addWatch(normalized.string(), &m_Listener, true);
            if (watchId < 0) {
                LW_ERROR("FileWatcherSystem [Watch]: efsw failed to watch: ", normalized.string());
                return -1;
            }
            m_DirToWatchId[normalized.string()] = watchId;
            LW_LOG("FileWatcherSystem [Watch]: new watchId=", watchId,
                     " for dir='", normalized.string(), "'");
        }
        ...
        LW_LOG("FileWatcherSystem [Watch]: registered subId=", subId,
                 " -> watchId=", watchId,
    ...
    void Unwatch(const int subscriptionId) {
        ...
        if (subIt == m_SubToWatchId.end()) {
            LW_WARN("FileWatcherSystem [Unwatch]: subId=", subscriptionId, " not found, ignoring.");
            return;
        }

        const efsw::WatchID watchId = subIt->second;
        LW_LOG("FileWatcherSystem [Unwatch]: removing subId=", subscriptionId,
        ...
        if (subs.empty()) {
            LW_LOG("FileWatcherSystem [Unwatch]: no more subs for watchId=", watchId, ", removing efsw watch.");
            ...
            for (auto it = m_DirToWatchId.begin(); it != m_DirToWatchId.end(); ++it) {
                if (it->second == watchId) {
                    LW_LOG("FileWatcherSystem [Unwatch]: removed dir entry '", it->first, "'");
    ...
    void Init() {
        if (m_Started) {
            LW_WARN("FileWatcherSystem [Init]: already started, skipping.");
            return;
        }

        m_Started = true;
        m_FileWatcher.watch();
        LW_LOG("FileWatcherSystem [Init]: efsw watcher started.");
    }
    ...
    void ProcessPendingEvents() {
        ...
        LW_LOG("FileWatcherSystem [Process]: draining ", eventsCopy.size(), " event(s).");

        while (!eventsCopy.empty()) {
            const auto& [watchId, path, type] = eventsCopy.front();

            LW_LOG("FileWatcherSystem [Process]: event path='", path.string(),
                     "' type=", static_cast<int>(type),
                     " watchId=", watchId);

            auto watchIt = m_WatchIdToSubs.find(watchId);
            if (watchIt != m_WatchIdToSubs.end()) {
                LW_LOG("FileWatcherSystem [Process]: dispatching to ",
                ...
                for (int subId : watchIt->second) {
                    ...
                    if (cbIt != m_Callbacks.end()) {
                        ...
                        LW_WARN("FileWatcherSystem [Process]: subId=", subId,
                                  " has no callback, skipping.");
                    }
                }
            }
            else {
                LW_WARN("FileWatcherSystem [Process]: no subscribers for watchId=", watchId,
...
private:
    ...
    public:
        ...
        void handleFileAction(
            ...
            LW_LOG("FileWatcherSystem [efsw raw]: watchId=", watchId,
                     " dir='", dir,
                     "' file='", filename,
            ...
            switch (action) {
                ...
                default:
                    LW_WARN("FileWatcherSystem [efsw raw]: unknown action=",
            ...
            LW_LOG("FileWatcherSystem [efsw raw]: enqueued '", fullPath.string(),
