...
#include "asset-management/private/AssetKey.h"
...
AssetManager::AssetManager(std::string rootPath) : m_Root(std::move(rootPath)) {
    CORE_ASSERT(!m_Root.empty(), "AssetManager: A Root path cannot be empty.");
}
...
std::string AssetManager::LoadText(const std::string& path) const {
    ...
    CORE_ASSERT(file.is_open(), "AssetManager: Could not open a file at: " + fullPath.string());
...
std::vector<uint8_t> AssetManager::LoadBytes(const std::string& path) const {
    ...
    CORE_ASSERT(file.is_open(), "AssetManager: Could not open a binary file at: " + fullPath.string());
...
Shared<Texture> AssetManager::GetTextureFromAbsolutePath(const std::string& absolutePath) {
	...
	CORE_ASSERT(file.is_open(), "AssetManager: Could not open the absolute texture path: " + absolutePath);
